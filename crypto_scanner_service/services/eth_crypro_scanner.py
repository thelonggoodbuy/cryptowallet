...
from crypto_scanner_service.services.abstract_crypto_scanner import (
    AbstractCryproScanner,
)
...
class EtheriumCryproScanner(AbstractCryproScanner):
    ...
    async def return_all_transactions_by_wallet(
        ...
        try:
            transactions_list = await aiotherscan_client.account.normal_txs(
                address=wallet_data["current_wallet_adress"]
            )
            transaction_dict = {}
            for transaction in transactions_list:
                ...
                transaction_dict[transaction["hash"]] = {
                    "send_from": self.w3_connection.to_checksum_address(
                        transaction["from"]
                    ),
                    "send_to": self.w3_connection.to_checksum_address(
                        transaction["to"]
                    ),
                    "value": transaction["value"],
                    "txn_hash": transaction["hash"],
                    "date_time_transaction": datetime.fromtimestamp(
                        int(transaction["timeStamp"])
                    ),
                    "txn_fee": self.w3_connection.from_wei(
                        int(transaction["cumulativeGasUsed"]), "ether"
                    ),
                    "status": status,
                }

            await self.compare_current_transactions_with_saved(
    ...
    async def compare_current_transactions_with_saved(
        transaction_dict: dict, wallet_data: dict
    ) -> None:
        """
        ...
        for transaction_obj in saved_transactions:
            if (
                transaction_obj.txn_hash in transaction_dict
                and transaction_obj.status == "pending"
            ):
                transaction_new_data = transaction_dict[transaction_obj.txn_hash]
                await TransactionETHService.update_transaction(
