...
class ETHParserService:
    ...
    async def update_transactions_from_parser(
        self, new_or_updated_transactions, send_to, send_from
    ):
        message_set = set()

        for transaction_hash in new_or_updated_transactions:
            transaction_obj = await TransactionETHService.create_or_update_transaction(
                transaction_hash
            )

            try:
                receiver_wallet_address = send_to[transaction_hash]
                sid_list = redis_parser_service.return_list_of_sids_per_address(
