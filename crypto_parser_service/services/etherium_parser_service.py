...
class ETHParserService:
    ...
    async def handle_block(cls, block_number: int) -> None:
        ...
        block_data = await cls.web3_async_connection.eth.get_block(
        ...
        wallets_addresses_dict = (
            await WalletEtheriumService.return_all_wallets_addresses()
        )

        # check if there are orders with status transaction pending
        order_dict = (
            await OrderEthService.return_orders_tied_with_pending_transactions()
        )

        for transaction in block_data.transactions:
            if transaction.get("from") in wallets_addresses_dict:
                transactions_set.add(transaction)
                value = cls.web3_async_connection.to_wei(
                    transaction.get("value"), "ether"
                )
                transaction_notification_from[transaction.get("hash").hex()] = {
                    "value": value,
            ...
            if transaction.get("to") in wallets_addresses_dict:
                ...
                transaction_notification_to[transaction.get("hash").hex()] = {
                    "summ": value,
        ...
        for transaction in transactions_set:
            saved_transaction = (
                await TransactionETHService.create_or_update_transaction(transaction)
            )
            ...
            if saved_transaction.txn_hash in transaction_notification_from:
                notification_dict = {
                    ...
                    "user_id": transaction_notification_from[
                        saved_transaction.txn_hash
                    ]["user_id"],
                    "wallet": transaction_notification_from[saved_transaction.txn_hash][
                        "wallet"
                    ],
                }
            ...
            if saved_transaction.txn_hash in transaction_notification_to:
                notification_dict = {
                    ...
                    "user_id": transaction_notification_to[saved_transaction.txn_hash][
                        "user_id"
                    ],
                    "wallet": transaction_notification_to[saved_transaction.txn_hash][
                        "wallet"
                    ],
                }
                await cls.send_notification(notification_dict)
            ...
            print("==============this is order dictionary in parser!================")
            print(order_dict)
            print("=================================================================")
            if saved_transaction.txn_hash in order_dict:
                print(
                    "<-----You want to change transaction for some order.you data is:----->"
                )
                print(order_dict[saved_transaction.txn_hash])
                order_dict["user_id"] = transaction_notification_from[
                    saved_transaction.txn_hash
                ]["user_id"]

                # TODO убрать колхоз и переписать на messaging
                from delivery_config.services.delivery_eth_service import (
                    DeliveryEthService,
                )
    ...
    async def send_notification(cls, notification_data: dict) -> None:
        ...
        if notification_data["type"] == "from":
            message = cls.sender_message.format(
                notification_data["summ"], notification_data["wallet"]
            )
            print("RECEIVER---MESSAGE-------->>>")
            print("--->notification_data-----")
            wallet_in_db = await WalletEtheriumService.return_wallet_per_address(
                notification_data["wallet"]
            )
            updated_balance = (
                await WalletEtheriumService.update_and_return_ballance_state(
                    wallet_in_db
                )
            )
            ...
            message = cls.receiver_message.format(
                notification_data["summ"], notification_data["wallet"]
            )
            print("RECEIVER---MESSAGE-------->>>")
            print("--->notification_data-----")
