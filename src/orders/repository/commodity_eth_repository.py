...
from src.orders.repository.commodity_abstract_repository import (
    CommodityAbstractRepository,
)
...
class CommodityEthRepository(CommodityAbstractRepository):
    async def save_commodity_in_db(self, commodity_data):
        ...
        async with async_session() as session:
            ...
            commodity = await session.execute(
                select(Commodity)
    ...
    async def return_commodity_by_id(self, commodity_id):
        ...
        async with async_session() as session:
            query = (
                ...
                .filter(Commodity.id == int(commodity_id))
                .limit(1)
            )
            result = await session.execute(query)
