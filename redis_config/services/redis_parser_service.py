...
class RedisParserService():
    ...
    async def link_sids_with_adress(self, sid, address):
        ...
        match sid_list:
            case None:
                online_sid_list = [sid, ]
                await self.redis_connection.hmset('wallet_sid_pair', mapping={address: str(online_sid_list)})
            case byte:
                decoded_sid_list = ast.literal_eval(sid_list.decode())
                decoded_sid_list.append(sid)
                await self.redis_connection.hmset('wallet_sid_pair', mapping={address: str(decoded_sid_list)})



    async def add_user_to_wallets_online(self, sid, all_users_wallets, user_id):
        await self.redis_connection.hmset('profile_wallets_seed_user_pair', mapping={sid: user_id})
        ...
        for wallet_key in all_users_wallets:
            ...
            # new!
            await self.link_sids_with_adress(sid, all_users_wallets[wallet_key]['address'])

        wallets_data = await self.redis_connection.hget("profile_wallets_online", user_id)

        match wallets_data:
            case None:
                wallets_data_dict = {'wallets_data': user_wallets_addresses, 'user_id': user_id, 'connection_quantity': 1, 'sid_list': [sid, ]}
                await self.redis_connection.hmset('profile_wallets_online', mapping={user_id: str(wallets_data_dict)})
...
# ============================================================================================================
    async def delete_user_from_wallets_online(self, sid):
        ...
        wallets_addresses = await WalletEtheriumService.return_all_wallets_adresses_per_user_id(user_id)
        ...
        if user_data_dict['connection_quantity'] <= 0:
            ...
            await self.redis_connection.hmset('profile_wallets_online', mapping={user_id: str(user_data_dict)})
        await self.redis_connection.close()
    ...
    async def return_user_id_per_sid_and_delete(self, sid):
        user_id = int((await redis_connection.hmget('profile_wallets_seed_user_pair', sid))[0])
